    ...
    >
      ...
      <div className="flex items-start gap-3">
        <div className="grid h-10 w-10 place-items-center rounded-xl bg-gradient-to-br from-sky-400/20 to-cyan-300/10 ring-1 ring-black/10 dark:ring-white/10">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" className="h-5 w-5 text-sky-700 dark:text-sky-300">
            <path fill="currentColor" d="M8 5v14l11-7-11-7Z" />
          </svg>
        </div>
        <div className="min-w-0">
          <div className="truncate text-lg font-semibold text-neutral-900 dark:text-white">{title}</div>
          {desc && <p className="mt-1 line-clamp-2 text-sm text-neutral-700 dark:text-white/70">{desc}</p>}
          {tag && (
...
function SectionTitle({ eyebrow, title, desc }: { eyebrow?: string; title: string; desc?: string }) {
  return (
    ...
    >
      {eyebrow && (
        <div className="mb-2 text-xs uppercase tracking-[0.2em] text-neutral-600 dark:text-white/60">{eyebrow}</div>
      )}
      <h2 className="text-3xl md:text-4xl font-semibold text-neutral-900 dark:text-white">{title}</h2>
      {desc && <p className="mt-3 text-neutral-700 dark:text-white/70">{desc}</p>}
    </motion.div>
  );
}
  ...
  return (
    <div className="min-h-screen w-full bg-white text-neutral-900 scroll-smooth dark:bg-neutral-950 dark:text-white">
      ...
      <section className="relative">
        <div className="mx-auto grid max-w-6xl items-center gap-8 px-4 py-14 md:grid-cols-[1.2fr_0.8fr]">
          <motion.div variants={fadeUp} initial="hidden" animate="show" className="space-y-5">
            <h1 className="text-4xl md:text-5xl font-semibold leading-[1.1] dark:text-white">{t("hero.title")}</h1>
            <p className="text-neutral-700 dark:text-white/70">{t("hero.text")}</p>
            <div className="flex gap-3 pt-2">
              <LocalizedLink href="/" className="rounded-xl border border-black/10 bg-white/60 px-5 py-3 text-sm hover:bg-white/80 dark:border-white/15 dark:bg-white/5 dark:hover:bg-white/10">
                {t("hero.back")}
              </LocalizedLink>
            </div>
          </motion.div>
          ...
          <div className="rounded-2xl border border-black/10 bg-white/70 p-6 backdrop-blur-xl shadow-xl dark:border-white/10 dark:bg-white/5">
            <div className="text-sm text-neutral-600 dark:text-white/70">{t("filters.title")}</div>
      ...
      <section className="py-8 md:py-10">
        <SectionTitle eyebrow={t("grid.eyebrow")} title={t("hero.title")} desc={t("grid.desc")} />
        <div className="mx-auto mt-8 grid max-w-6xl gap-4 px-4 sm:grid-cols-2 md:grid-cols-3">
          <CategoryLinkCard title={labels.history} tag={t("tags.history")} desc={t("history.0.desc")} href="/materials/history" />
          <CategoryLinkCard title={labels.traditions} tag={t("tags.traditions")} desc={t("traditions.0.desc")} href="/materials/traditions" />
          <CategoryLinkCard title={labels.language} tag={t("tags.language")} desc={t("language.0.desc")} href="/materials/language" />
          <CategoryLinkCard title={labels.cuisine} tag={t("tags.cuisine")} desc={t("cuisine.0.desc")} href="/materials/cuisine" />
          <CategoryLinkCard title={labels.music} tag={t("tags.music")} desc={t("music.0.desc")} href="/materials/music" />
        </div>
      </section>

      {/* Доп. подборки — оставим, но как лаконичные ссылки */}
      <section className="py-12 md:py-16">
        <SectionTitle eyebrow={t("collections.eyebrow")} title={t("collections.title")} desc={t("collections.desc")} />
        <div className="mx-auto mt-8 grid max-w-6xl gap-4 px-4 md:grid-cols-2">
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                <div className="text-xs text-neutral-600 dark:text-white/60">{t("collections.playlist")}</div>
                <div className="text-lg font-medium dark:text-white">{t("collections.playlistA.title")}</div>
              </div>
              <span className="rounded-full border border-black/10 bg-black/5 px-3 py-1 text-xs dark:border-white/15 dark:bg-white/10">
                8 {t("labels.items")}
              </span>
            </div>
            <p className="mt-2 line-clamp-2 text-sm text-neutral-700 dark:text-white/70">{t("collections.playlistA.desc")}</p>
          </LocalizedLink>
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                <div className="text-xs text-neutral-600 dark:text-white/60">{t("collections.playlist")}</div>
                <div className="text-lg font-medium dark:text-white">{t("collections.playlistB.title")}</div>
              </div>
              <span className="rounded-full border border-black/10 bg-black/5 px-3 py-1 text-xs dark:border-white/15 dark:bg-white/10">
                6 {t("labels.items")}
              </span>
            </div>
            <p className="mt-2 line-clamp-2 text-sm text-neutral-700 dark:text-white/70">{t("collections.playlistB.desc")}</p>
          </LocalizedLink>
        </div>
      </section>
